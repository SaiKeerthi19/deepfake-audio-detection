...
|-------|----------|-----|---------|--------|-------|-----|-----|-----|-----|
| **W2V+SVM** | **768 wav2vec2** | **0.963** | **0.990** | **0.968** | **0.979** | **0.990** | **—** | **2,026** | **597** |
| W2V+LR | 768 wav2vec2 | 0.958 | 0.994 | 0.959 | 0.976 | 0.987 | — | 2,619 | 351 |
...
The most striking result is that **feature quality matters more than classifier sophistication**. Wav2Vec2 embeddings with a simple Logistic Regression (W2V+LR: 95.8% accuracy, 3.6% avg EER) dramatically outperform a CNN trained on Mel spectrograms (56.3% accuracy, 15.0% avg EER), despite the CNN being a far more complex model. The 768-dim Wav2Vec2 representations, learned from 960 hours of unlabeled speech, capture fundamental properties of natural speech that synthetic audio fails to replicate.

### B. Generalization to Unseen Attacks

The evaluation set exclusively contains attack types (A07–A19) never seen during training. The steep performance drop of MFCC-based models (avg EER 14–20%) compared to Wav2Vec2 (avg EER 3.6%) indicates that hand-crafted features encode attack-specific patterns that don't transfer, while Wav2Vec2's learned representations capture more generalizable speech characteristics.

### C. Model Complementarity

No model achieves best EER on all 13 attacks simultaneously. CNN achieves 0.0% EER on A07/A08/A16 but 51.0% on A13, while Wav2Vec2+LR shows consistent 1–8% EER across all attacks. An ensemble combining CNN's strengths on certain attacks with Wav2Vec2's consistent generalization could further improve robustness.

### D. CNN Performance Analysis

The CNN's low overall accuracy (56.3%) despite high AUC (0.888) suggests a threshold calibration issue exacerbated by the class imbalance. The model achieves near-perfect precision (99.98%) but very low recall, indicating extreme conservatism — only classifying a sample as spoof when highly confident. This produces almost zero false alarms (FP=7) but misses 48.7% of all deepfakes. Per-attack analysis reveals extreme variability: near-perfect on some attacks (A07, A08, A16) but near-random on others (A13: 51% EER), suggesting the CNN learned attack-specific spectral patterns rather than general spoofing indicators.
...
**Key takeaways:**
1. Pre-trained speech representations generalize far better to unseen attacks than hand-crafted features.
2. The hardest attacks (A17, A18) become manageable with Wav2Vec2 (EER drops from 40%+ to 2–7%).
3. No single model dominates all attack types — ensemble approaches are recommended for deployment.
4. From a cybersecurity perspective, minimizing false negatives should take priority over minimizing false alarms.

**Future work** includes: fine-tuning Wav2Vec2 end-to-end, exploring ensemble fusion of CNN and Wav2Vec2 models, evaluating on ASVspoof 2021 and in-the-wild datasets, and investigating real-time deployment with model quantization.
